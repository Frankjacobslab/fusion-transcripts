...
################################################################################
# Script: bowtie2_alignment_hipsci.py
...
def process_sample(rna_seq_fastq):
    ...
    subprocess.run(["bowtie2", "--local", "--no-unal", "-N", "1", "--score-min", "G,32,21", "-x", bowtie2_index_base, "-U", rna_seq_fastq, "-S", aligned_sam], check=True) # First time G,29,16, second time G,32,21
